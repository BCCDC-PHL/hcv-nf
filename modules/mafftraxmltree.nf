process mafftraxmltree {
  ...
  if [ -s ${sample_id}_core_consensus.fa ]; then
    ...
    mafft --reorder --adjustdirection --anysymbol --thread 4 --auto mafftinput_core.fa > mafftoutput_core
    raxmlHPC -f d -p 12345 -s mafftoutput_core -m GTRCAT -n ${sample_id}_core
  fi

  if [ -s ${sample_id}_ns5b_consensus.fa ]; then
    cat ${params.ref_ns5b} ${sample_id}_ns5b_consensus.fa > mafftinput_ns5b.fa
    mafft --reorder --adjustdirection --anysymbol --thread 4 --auto mafftinput_ns5b.fa > mafftoutput_ns5b
    raxmlHPC -f d -p 12345 -s mafftoutput_ns5b -m GTRCAT -n ${sample_id}_ns5b
  fi
