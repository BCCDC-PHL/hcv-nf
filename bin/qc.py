...
code was adapted from https://github.com/BCCDC-PHL/mpxv-artic-nf/blob/main/bin/qc.py
"""
...
def make_qc_plot(depth_pos, n_density, ref_length, samplename, min_depth, amplicons, window=200, ylim_top=10**5, width_inches=20.0, height_inches=4.0):
    ...
    for amplicon_num, amplicon in amplicons.items():
        amplicon_rectangle = Rectangle((amplicon['start'], int("2")), amplicon['length'], 1, color=amplicon_pool_colors["2"])
...
def primers_to_amplicons(primers):
    amplicons = {}
    positive_orientation_primers = [primers[p] for p in primers if primers[p]['orientation'] == 'forward']
