...
p1 <- ggplot(core, aes(x=position, y=depth)) +
  ...
  facet_wrap(~ segment, ncol = 1) + ggtitle(paste0(filename," - core - top 3 reference mapped"))
...
p2 <- ggplot(ns5b, aes(x=position, y=depth)) +
  ...
  facet_wrap(~ segment, ncol = 1) + ggtitle(paste0(filename," - ns5b - top 3 reference mapped"))
